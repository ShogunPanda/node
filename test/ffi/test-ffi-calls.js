...
test('ffi string signatures convert strings to temporary pointers', () => {
  const { lib, functions } = ffi.dlopen(libraryPath, {
    string_length: { parameters: ['string'], result: 'u64' },
    safe_strlen: { parameters: ['str'], result: 'i32' },
  });
  try {
...
test('ffi buffer and ArrayBuffer signatures pass backing-store pointers', () => {
  {
    const { lib, functions } = ffi.dlopen(libraryPath, {
      first_byte: { parameters: ['buffer'], result: 'u8' },
    });
    try {
  ...
  {
    const { lib, functions } = ffi.dlopen(libraryPath, {
      first_byte: { parameters: ['arraybuffer'], result: 'u8' },
    });
    try {
