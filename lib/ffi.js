...
function wrapFFIFunction(rawFn, parameters, resultType, owner) {
  if (parameters === undefined && rawFn !== undefined && rawFn !== null) {
    parameters = rawFn[kSbArguments] ?? rawFn[kFastParams];
  }
  initializeFastBufferMetadata(rawFn, parameters);
  const wrapped = wrapWithSharedBuffer(rawFn, parameters, resultType);
  if (wrapped !== rawFn) {
    return wrapped;
  }
  return wrapWithRawPointerConversions(rawFn, parameters, owner);
}

// Every public path that surfaces native-created FFI functions goes through the
// wrapper dispatcher here. The SharedBuffer and Fast API modules only expose
// their own wrapping primitives; this file decides which one applies.
...
DynamicLibrary.prototype.getFunction = function getFunction(name, sig) {
  ...
  return wrapFFIFunction(raw, sigParams(sig), sigResult(sig), this);
};
...
DynamicLibrary.prototype.getFunctions = function getFunctions(definitions) {
  ...
  for (let i = 0; i < keys.length; i++) {
    const name = keys[i];
    // No `definitions`: native side returned every cached function, so use
    // each raw function's hidden metadata to decide whether it needs wrapping.
    if (definitions === undefined) {
      out[name] = wrapFFIFunction(raw[name], undefined, undefined, this);
    } else {
      const sig = definitions[name];
      out[name] = wrapFFIFunction(
        raw[name], sigParams(sig), sigResult(sig), this);
    }
  }
  return out;
};
...
{
  // The native side installs `functions` as an accessor returning raw cached
  // functions. Rewrap each access so both SharedBuffer and Fast API pointer
  // conversion wrappers are applied consistently.
