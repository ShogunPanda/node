...
MaybeLocal<Function> DynamicLibrary::CreateFunction(
  ...
  // The generated JS function owns FFIFunctionInfo through a weak callback.
  // The persistent library reference keeps the dlopen handle alive while any
  // generated function can still call into it.
...
static void Initialize(Local<Object> target,
  ...
  target
      ->Set(context,
            FIXED_ONE_BYTE_STRING(isolate, "kFastParams"),
            env->ffi_fast_params_symbol())
      .Check();
  target
      ->Set(context,
